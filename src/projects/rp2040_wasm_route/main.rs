...
#[cfg(all(target_arch = "arm", target_os = "none"))]
const ALERT_THRESHOLD: u32 = 0x1000_0000;
#[cfg(all(target_arch = "arm", target_os = "none"))]
...
async fn supervisor_session(endpoint: &mut SupervisorEndpoint) {
    ...
    let sample = match endpoint.recv::<Msg<LABEL_SAMPLE_REQ, u32>>().await {
        Ok(sample) => sample,
        Err(_) => fail_closed("[core0] recv sample"),
    };
    uart_hex_line("[core0] recv sample 0x", sample);
    if sample >= ALERT_THRESHOLD {
        ...
        match control {
            Ok(()) => {}
            _ => fail_closed("[core0] route publish alert"),
        }
        match endpoint
            ...
            .await
        {
            Ok(_) => uart_hex_line("[core0] sent publish alert 0x", sample),
            Err(_) => fail_closed("[core0] send publish alert"),
        }
    } else {
        let control = endpoint
        ...
        match control {
            Ok(()) => {}
            _ => fail_closed("[core0] route publish normal"),
        }
        match endpoint
            ...
            .await
        {
            Ok(_) => uart_hex_line("[core0] sent publish normal 0x", sample),
            Err(_) => fail_closed("[core0] send publish normal"),
        }
    }

    uart_line("[core0] wait yield");
    match endpoint.recv::<Msg<LABEL_YIELD_REQ, ()>>().await {
        Ok(()) => uart_line("[core0] recv yield"),
        Err(_) => fail_closed("[core0] recv yield"),
    }
    match endpoint
        ...
        .await
    {
        Ok(_) => uart_line("[core0] sent yield ack"),
        Err(_) => fail_closed("[core0] send yield ack"),
    }
}
...
async fn engine_session(endpoint: &mut EngineEndpoint) {
        ...
        match trap {
            GuestTrap::HostCall(request) => match request {
                EngineReq::LogU32(value) => {
                    uart_hex_line("[core1] wasm trap sample 0x", value);
                    match endpoint
                    ...
                    {
                        Ok(_) => {}
                        Err(_) => fail_closed("[core1] send sample"),
                    }
                    let branch = match endpoint.offer().await {
                        Ok(branch) => branch,
                        Err(_) => fail_closed("[core1] offer publish"),
                    };
                    match branch.label() {
                        LABEL_PUBLISH_NORMAL => {
                            let published =
                                match branch.decode::<Msg<LABEL_PUBLISH_NORMAL, u32>>().await {
                                    Ok(published) => published,
                                    Err(_) => fail_closed("[core1] decode publish normal"),
                                };
                            if published != value {
                                fail_closed("[core1] publish normal mismatch");
                            }
                            uart_hex_line("[core1] offer publish normal 0x", published);
                            guest
                                .complete_host_call(EngineRet::Logged(published))
                                .unwrap_or_else(|_| fail_closed("[core1] complete normal sample"));
                        }
                        LABEL_PUBLISH_ALERT => {
                            let published =
                                match branch.decode::<Msg<LABEL_PUBLISH_ALERT, u32>>().await {
                                    Ok(published) => published,
                                    Err(_) => fail_closed("[core1] decode publish alert"),
                                };
                            if published != value {
                                fail_closed("[core1] publish alert mismatch");
                            }
                            uart_hex_line("[core1] offer publish alert 0x", published);
                            guest
                                .complete_host_call(EngineRet::Logged(published))
                                .unwrap_or_else(|_| fail_closed("[core1] complete alert sample"));
                        }
                        _ => fail_closed("[core1] unexpected publish branch"),
                    }
                }
                EngineReq::Yield => {
                    ...
                    match endpoint.recv::<Msg<LABEL_YIELD_RET, ()>>().await {
                        Ok(()) => uart_line("[core1] recv yield ack"),
                        Err(_) => fail_closed("[core1] recv yield ack"),
                    }
                    guest
                        .complete_host_call(EngineRet::Yielded)
                        .unwrap_or_else(|_| fail_closed("[core1] complete yield"));
                }
    ...
    uart_line("[core1] hibana wasm route ok");
}

#[cfg(all(target_arch = "arm", target_os = "none"))]
fn core0_main() -> ! {
    uart_init();
    uart_line("[core0] hibana wasm route");
