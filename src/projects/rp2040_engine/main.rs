...
#[cfg(all(target_arch = "arm", target_os = "none"))]
static CORE0_PROGRAM: RoleProgram<0> = project(&engine_program!());
#[cfg(all(target_arch = "arm", target_os = "none"))]
static CORE1_PROGRAM: RoleProgram<1> = project(&engine_program!());
...
async fn supervisor_session(endpoint: &mut SupervisorEndpoint) {
    uart_line("[core0] wait engine request");
...
async fn engine_session(endpoint: &mut EngineEndpoint) {
    ...
    uart_hex_line("[core1] req log_u32 0x", ENGINE_LOG_VALUE);
    match endpoint
    ...
    {
        Ok(_) => {}
        Err(_) => fail_closed("[core1] send log_u32"),
    }
    ...
    match reply {
        EngineRet::Logged(value) => {
            uart_hex_line("[core1] ret logged 0x", value);
            if value != ENGINE_LOG_VALUE {
                fail_closed("[core1] logged value mismatch");
            }
        }
        EngineRet::Yielded => fail_closed("[core1] expected logged first"),
        EngineRet::Wasip1StdoutWritten(_) => fail_closed("[core1] expected logged first"),
        EngineRet::Wasip1StderrWritten(_) => fail_closed("[core1] expected logged first"),
        EngineRet::Wasip1StdinRead(_) => fail_closed("[core1] expected logged first"),
        EngineRet::Wasip1ClockNow(_) => fail_closed("[core1] expected logged first"),
        EngineRet::Wasip1RandomSeed(_) => fail_closed("[core1] expected logged first"),
        EngineRet::TimerSleepDone(_) => fail_closed("[core1] expected logged first"),
        EngineRet::GpioSetDone(_) => fail_closed("[core1] expected logged first"),
        _ => fail_closed("unexpected wasi p1 reply"),
    }

    let request = EngineReq::Yield;
    uart_line("[core1] req yield");
    match endpoint
        .flow::<Msg<LABEL_ENGINE_REQ, EngineReq>>()
        .expect("engine flow<req>")
        .send(&request)
    ...
    {
        Ok(_) => {}
        Err(_) => fail_closed("[core1] send yield"),
    }
    ...
    match reply {
        EngineRet::Yielded => uart_line("[core1] ret yielded"),
        EngineRet::Logged(_) => fail_closed("[core1] expected yielded second"),
        EngineRet::Wasip1StdoutWritten(_) => fail_closed("[core1] expected yielded second"),
        EngineRet::Wasip1StderrWritten(_) => fail_closed("[core1] expected yielded second"),
        EngineRet::Wasip1StdinRead(_) => fail_closed("[core1] expected yielded second"),
        EngineRet::Wasip1ClockNow(_) => fail_closed("[core1] expected yielded second"),
        EngineRet::Wasip1RandomSeed(_) => fail_closed("[core1] expected yielded second"),
        EngineRet::TimerSleepDone(_) => fail_closed("[core1] expected yielded second"),
        EngineRet::GpioSetDone(_) => fail_closed("[core1] expected yielded second"),
        _ => fail_closed("unexpected wasi p1 reply"),
    }
    ...
    uart_line("[core1] hibana engine supervisor ok");
}

#[cfg(all(target_arch = "arm", target_os = "none"))]
fn core0_main() -> ! {
    uart_init();
    uart_line("[core0] hibana engine supervisor");
